...
describe("error states", () => {
  ...
  test("if user submits form with some empty fields", () => {
    ...
    userEvent.type(screen.getByRole("textbox", { name: /first name/i }), fakeUserSignUpData.first_name);

    userEvent.click(screen.getByRole("button", { name: /sign up/i }));

    expect(screen.getAllByText(/please fill out field/i)).toHaveLength(4);

    userEvent.type(screen.getByRole("textbox", { name: /last name/i }), fakeUserSignUpData.last_name);

    userEvent.click(screen.getByRole("button", { name: /sign up/i }));

    expect(screen.getAllByText(/please fill out field/i)).toHaveLength(3);

    userEvent.type(screen.getByRole("textbox", { name: /email/i }), fakeUserSignUpData.email);

    userEvent.click(screen.getByRole("button", { name: /sign up/i }));

    expect(screen.getAllByText(/please fill out field/i)).toHaveLength(2);

    userEvent.type(screen.getByLabelText(/^password/i), fakeUserSignUpData.password);
  ...
  test("if user submits form with password less than 6 characters in length", () => {
    ...
    expect(screen.getByText(/password must be a minimum of 6 characters/i)).toBeInTheDocument();
  });
  ...
  test("if user submits form with previously registered email", async () => {
    ...
    userEvent.type(screen.getByRole("textbox", { name: /email/i }), fakeUserSignUpData.email);
    userEvent.type(screen.getByLabelText(/^password/i), fakeUserSignUpData.password);
    userEvent.type(screen.getByLabelText(/^confirm password/i), fakeUserSignUpData.confirm_password);

    userEvent.click(screen.getByRole("button", { name: /sign up/i }));

    expect(await screen.findByText(/a user already exists with this email/i)).toBeInTheDocument();
  });
});
...
describe("golden path", () => {
  test("if user successfully submits form", async () => {
    ...
    await waitFor(() => {
      expect(history.location.pathname).toBe(`/user/${fakeFirebaseUserData.user.uid}`);
    });
  });
});
